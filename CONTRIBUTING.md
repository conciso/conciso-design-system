...
- **Getönte `-50`-Flächen im Dark** sind gedämpfte, *getönte* Dunkeltöne (nicht near-black) und liegen **über** `bg-page`, nicht darunter. Sie trennen sich vom Seitengrund über den **Farbton** (1,06:1 Helligkeitsabstand), Karten darauf halten 1,27:1. Grund: `bg-page` trägt 17,5:1 gegen Weiß, reines Schwarz nur 21:1, nach unten bleibt Faktor 1,20. Für eine Stufe unterhalb der Seite ist kein Platz mehr. Sättigung dabei **unter 0,70** halten (aktuell 0,60), sonst „vibriert" die Fläche großflächig auf dunklem Grund.
...
- **Der Dark-Token-Block steht in `@media screen`.** Im Druck greifen dadurch die Light-Werte aus `tokens.css`, ohne dass sie ein zweites Mal gepflegt werden müssen. Neue Dark-Tokens gehören in diesen Block, sonst drucken sie dunkel mit.
