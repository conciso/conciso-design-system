...
- **Kartentragende `n-50`-Sektion braucht keine Zusatzklasse.** Im Dark werden `n-50`-Sektionen auf `bg-surface` gehoben, damit der Light-Rhythmus erhalten bleibt. Trägt so eine Sektion selbst Flächen auf `bg-surface` (Karten, Tabellen, Formularfelder, inline gesetzte Kästen), kollidiert sie mit ihnen auf demselben Ton (1,00:1, es bleibt nur die `--bd`-Haarlinie). Diesen Fall erkennt `dark-mode.css` per `:has()` und lässt die Sektion auf `bg-page`, sodass die Karten die hellere Stufe bilden. **Kommt eine neue Komponente mit `bg-surface`-Fläche dazu, gehört ihr Selektor in diese `:has()`-Liste.** `.ep-section-cards` bleibt als ausdrücklicher Override (Inhalt, den kein Selektor sieht; Browser ohne `:has()`), ist aber nicht mehr Pflicht.
