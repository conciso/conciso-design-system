...
- **Nur [veröffentlichungsrelevante Commits](CONTEXT.md#veröffentlichungsrelevanter-pfad) zählen.** Ein Commit zählt, wenn er mindestens einen Pfad aus `scripts/release/relevant-paths.mjs` berührt (ausgelieferter Inhalt beider Pakete plus dessen Build-Eingaben). Ein Commit, der nur Storybook, Beispielseiten, Doku oder CI ändert, ist für Release und Commit-Konvention unsichtbar.
- **Der Conventional-Commit-Typ entscheidet die Stufe** (nur für relevante Commits): `feat` → Minor, `fix`/`perf`/`build(deps)` → Patch, ein `!` am Typ oder ein `BREAKING CHANGE:`-Footer → Major, jeder andere Typ → kein Release. Scopes sind frei und wirken sich auf die Stufe nicht aus.
